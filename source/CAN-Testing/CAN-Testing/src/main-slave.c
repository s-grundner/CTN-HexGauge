...
// Encoder
#define EC_PIN_A GPIO_NUM_25
#define EC_PIN_B GPIO_NUM_26
#define EC_PIN_SW GPIO_NUM_19
...
static void twai_transmit_task(void* arg)
{
	...
	encoder_handle_t ec = (encoder_handle_t)arg;
	...
	while (1)
	{
		...
		else if (action == TX_SEND_DATA)
		{
			...
			while (1)
			{
				...
				u_int32_t sensor_data = abs(encoder_read(ec));
				for (int i = 0; i < 4; i++)
				{
...
{
	encoder_handle_t ec;
	encoder_config_t ec_config = {
			.pin_a = EC_PIN_A,
			.pin_b = EC_PIN_B,
			.pin_sw = EC_PIN_SW
	};

	encoder_init(&ec, &ec_config);
	...
	xTaskCreatePinnedToCore(twai_transmit_task, "TWAI_tx", 4096, ec, TX_TASK_PRIO, NULL, tskNO_AFFINITY);
